	   ...
	   static final String FICHIER_XSD_V111 = "XMLNotificationOvinCaprinV111.XSD";
	    ...
	    static {
	        ...
	        try {
	            ...
	            e.printStackTrace();
	            log.error("Erreur lors de la récupération dans le fichier de config confosimoc");
	            log.error("Erreur", e);
	    ...
	    public static void main(String[] args) {
            ...
            while (true) {
                ...
                try {
    	            log.info("Traitement du fichier : " + fichier.getName());
    	            log.info("[1] Recherche du schéma de référence du fichier xml");
                    ...
                    if (fichierXsd == null) {
                    	...
                    	log.info("[2] Contrôle des codes ASCII");
                        ...
                        if (XmlUtils.ControlerCodeASCII(fichier.getPath(), erreurs, versionSchemaReference)) {
                        	log.info("[3] Conversion du fichier xml");
                            ...
                            log.info("[4] Contrôle de cohérence des données");
                    	...
                    	log.info("Rejet du fichier " + fichier.getName() + " : erreur de nommage");
                        System.out.println("Rejet du fichier " + fichier.getName() + " : erreur de nommage");
                        erreurs.add("erreur de nommage");
                        fichierValide = false;
                    }
                } catch (Exception e3) {
                	log.error("Erreur inattendue sur le fichier " + fichier.getName());
                ...
                if (!fichierValide) {
                    ...
                    if (erreurs.size() == 0) {
                        ...
                        log.info("Création du fichier de réponse " + file.getPath());
                        ...
                        if (mvtOC != null) {
                            ...
                            log.info("Reécriture du fichier xml " + file.getName());
                        ...
                        log.info("Ecriture du fichier log " + file2.getPath());
                ...
                if (versionSchemaReference != null && "V111".equals(versionSchemaReference) && !dateV2depassee.booleanValue()) {
                    ...
                    log.info("Ecriture du fichier log " + file3.getPath());
	    ...
	    public Resultat valideFile(String filename) {
            ...
            try {
                ...
                log.error("Erreur inattendue sur le fichier " + fichier.getName(), e3);
                StringBuffer sb = new StringBuffer();
                StackTraceElement[] st = e3.getStackTrace();
                int length2 = st.length;
                for (int i3 = 0; i3 < length2; i3++) {
            ...
            if (!fichierValide) {
                ...
                if (erreurs.size() == 0) {
                    ...
                    if (mvtOCV2 != null) {
                        ...
                        log.info("Reécriture du fichier xml " + file.getName());
                        XmlUtils.ecrireFichierXMLV2(jc, mvtOCV2, file.getPath());
                        if (! mvtOCV2.getCirculationOrienteeChargement().isEmpty())
                        ...
                        if (! mvtOCV2.getCirculationOrienteeDechargement().isEmpty())
                        	mvtOCV2.getCirculationOrienteeDechargement().get(0).getValidation().getAnomalie().forEach( 
                        		(Anomalie anomalie) -> {
                        			anomalies.add(anomalie.getCodeAnomalie().value() + ":" + anomalie.getLibelleAnomalie() );
                        			xmlAnomalies.add(anomalie);
                        		});
